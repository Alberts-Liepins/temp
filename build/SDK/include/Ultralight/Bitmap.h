...
class UExport Bitmap : public RefCounted {
 public:
  ...
  ///
  /// @param  should_copy  Whether or not a copy should be made of the pixels. If this is false, the
  ///                      returned Bitmap will use the raw pixels passed in as its own, but you are
  ///                      still responsible for destroying your buffer afterwards.
  ///
  /// @return  A ref-pointer to a new Bitmap instance.
  ///
  static Ref<Bitmap> Create(uint32_t width, uint32_t height, BitmapFormat format,
  ...
  ///
  virtual bool DrawBitmap(IntRect src_rect, IntRect dest_rect, Ref<Bitmap> src, bool pad_repeat)
      = 0;
  ...
  ///
  /// @param  convert_to_straight_alpha  The PNG format expects semi-transparent values to be stored
  ///                                    as straight alpha instead of premultiplied alpha, set this
  ///                                    to true to perform the conversion automatically.
  ///
  ...
  ///
  /// @param  high_quality  Whether or not a high quality resampling will be used during the resize.
  ///                       (Otherwise, just uses fast nearest-neighbor sampling)
  ///
  /// @return  Whether or not the operation succeeded. This operation is only valid if both formats
  ///          are kBitmapFormat_BGRA8_UNORM_SRGB and both the source and destination are non-empty.
  ///
  ...
  ///
  /// @note  Only valid if the format is kBitmapFormat_BGRA8_UNORM_SRGB
  ///
